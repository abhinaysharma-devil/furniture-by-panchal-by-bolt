...
import { furnitureItems, categories } from '../data/mockData';
...
const ProductDetail: React.FC = () => {
  ...
  const [relatedProducts, setRelatedProducts] = useState(furnitureItems.slice(0, 4));
  const { addItem } = useCartStore();

  useEffect(() => {
    if (slug) {
      const foundProduct = furnitureItems.find(item => item.slug === slug);
      setProduct(foundProduct);
      
      if (foundProduct) {
        // Find related products from the same category
        const categoryProducts = furnitureItems.filter(
          item => item.categoryId === foundProduct.categoryId && item.id !== foundProduct.id
        );
