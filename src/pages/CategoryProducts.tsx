...
import { useGetProductCategory, useGetProducts } from '../apis/apiHooks';
...
const CategoryProducts: React.FC = () => {
  ...
  useEffect(() => {
    if (getProductCategory) {
      let category = getProductCategory?.find(cat => cat.slug === slug)
      setCategory(category);
    }
  }, [getProductCategory]);


  if (isProductsByCategoryLoading) return <Skeleton active />;
  if (productError) return <p>Error: {productError.message}</p>;

  const handleSort = (sortType: typeof sortBy) => {
    setSortBy(sortType);

    let sortedProducts = [...getProductsByCategory];
  ...
  return (
    <div className="py-16">
      <div className="container-custom">
        <div className="mb-8">
          <Link to="/categories" className="link flex items-center mb-4" onClick={navBack}>
